...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _initializeAvailableMonths() {
    setState(() {
      debugPrint('[HomeScreen] Initializing available months.');
      ...
      debugPrint(
        '[HomeScreen] Retrieved expense list: ${expenseList.length} items.',
      );

      if (expenseList.isEmpty) {
        debugPrint(
          '[HomeScreen] Expense list is empty. Setting availableMonths to current month.',
        );
        ...
        debugPrint(
          '[HomeScreen] Earliest month: $earliest, Latest month: $latest.',
        );
        ...
        while (current.isBefore(lastDay) || current.isAtSameMomentAs(lastDay)) {
          availableMonths.add(current);
          current = DateTime(current.year, current.month + 1, 1);
        }

        debugPrint(
          '[HomeScreen] Generated available months: $availableMonths.',
        );

        // Set selected month to latest month
        if (availableMonths.isNotEmpty) {
          selectedMonth = availableMonths.last;
          debugPrint('[HomeScreen] Selected month set to: $selectedMonth.');
        }
      }
    });
  }
  ...
  List<ExpenseItem> _getExpensesForSelectedMonth(List allExpenses) {
    ...
    debugPrint('[HomeScreen] Filtered expenses for $selectedMonth: $filtered');
    return filtered;
  }
